...
path = '/usr/local/airflow/dags/kw3/' ## airflow
#path = '/Users/konradburchardt/Desktop/docker-airflow/dags'  ##local
os.chdir(path)
files = sorted(glob.glob(os.path.join(path,'kw3.csv')))
...
path2 = '/usr/local/airflow/dags/kw3/rankings/' ## airflow
...
#exporting to CSV
export_csv = ranks.to_csv (r'/usr/local/airflow/dags/kw3/kw3.csv', index =True,index_label = 'id', header=True)
