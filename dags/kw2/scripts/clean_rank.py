...
path = '/usr/local/airflow/dags/kw2/' ## airflow
#path = '/Users/konradburchardt/Desktop/docker-airflow/dags'  ##local
os.chdir(path)
files = sorted(glob.glob(os.path.join(path,'kw2.csv')))
...
path2 = '/usr/local/airflow/dags/kw2/rankings/' ## airflow
...
#exporting to CSV
export_csv = ranks.to_csv (r'/usr/local/airflow/dags/kw2/kw2.csv', index =True,index_label = 'id', header=True)
