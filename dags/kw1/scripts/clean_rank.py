...
path = '/usr/local/airflow/dags/kw1/' ## airflow
#path = '/Users/konradburchardt/Desktop/docker-airflow/dags'  ##local
os.chdir(path)
files = sorted(glob.glob(os.path.join(path,'kw1.csv')))
...
path2 = '/usr/local/airflow/dags/kw1/rankings/' ## airflow
...
#exporting to CSV
export_csv = ranks.to_csv (r'/usr/local/airflow/dags/kw1/kw1.csv', index =True,index_label = 'id', header=True)
