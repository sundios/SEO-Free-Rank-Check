...
os.chdir(path)
files = sorted(glob.glob(os.path.join(path,'ranks.csv')))
...
path2 = '/usr/local/airflow/dags/rankings/' ## airflow
...
#exporting to CSV
export_csv = ranks.to_csv (r'/usr/local/airflow/dags/ranks.csv', index =True,index_label = 'id', header=True)
